...
public class MedicalDevicesHub : Hub
{
    ...
    /// <summary>
    /// Solicita que o paciente envie os sinais vitais atualizados
    /// </summary>
    public async Task RequestVitalsUpdate(string appointmentId)
    {
        var userId = Context.User?.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        
        _logger.LogInformation(
            "MedicalDevicesHub: Médico {UserId} solicitou atualização de vitais na sala {AppointmentId}",
            userId ?? "unknown", appointmentId);
        
        await Clients.OthersInGroup($"appointment_{appointmentId}")
            .SendAsync("VitalsUpdateRequested", new
            {
                requestedBy = userId,
                timestamp = DateTime.UtcNow
            });
