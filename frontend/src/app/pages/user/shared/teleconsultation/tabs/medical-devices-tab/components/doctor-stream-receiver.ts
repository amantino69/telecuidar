  ...
  template: `
    <div class="doctor-stream-receiver">
      <div class="panel-header">
        <h4>
          <app-icon [name]="getDisplayStreamType() === 'auscultation' ? 'mic' : 'video'" [size]="20" />
          {{ getStreamTitle() }}
        </h4>
      ...
      @if (!hasActiveStream) {
        <div class="waiting-state">
          <div class="waiting-icon">
            <app-icon [name]="expectedStreamType === 'auscultation' ? 'mic' : 'video'" [size]="48" />
          </div>
...
export class DoctorStreamReceiverComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  @Input() expectedStreamType: 'auscultation' | 'video' = 'auscultation';
  ...
  ngOnInit(): void {
    console.log('[DoctorStreamReceiver] ngOnInit - appointmentId:', this.appointmentId, 'expectedStreamType:', this.expectedStreamType);
    
    // Conecta ao hub
    if (this.appointmentId) {
      console.log('[DoctorStreamReceiver] Conectando ao hub...');
    ...
    this.subscriptions.add(
      this.syncService.isConnected$.subscribe(connected => {
        console.log('[DoctorStreamReceiver] Hub conectado:', connected);
      })
    );
    ...
    this.subscriptions.add(
      this.syncService.remoteStream$.subscribe(stream => {
        console.log('[DoctorStreamReceiver] remoteStream$ recebido:', !!stream);
    ...
    this.subscriptions.add(
      this.syncService.streamType$.subscribe(type => {
        console.log('[DoctorStreamReceiver] streamType$ recebido:', type);
  ...
  private handleRemoteStream(stream: MediaStream | null): void {
    console.log('[DoctorStreamReceiver] handleRemoteStream chamado:', {
      ...
      currentStreamType: this.streamType,
      tracks: stream?.getTracks().map(t => ({ kind: t.kind, enabled: t.enabled, readyState: t.readyState }))
    });

    // Se não há stream, limpa
    if (!stream) {
    ...
    console.log('[DoctorStreamReceiver] Stream info:', { 
      hasVideo, 
      hasAudio,
      expectedStreamType: this.expectedStreamType 
    });

    // Verifica se o tipo do stream corresponde ao que este componente espera
    // Se esperamos 'auscultation', o stream deve ter áudio
    // Se esperamos 'video', o stream deve ter vídeo
    const isMatchingType = 
      (this.expectedStreamType === 'auscultation' && hasAudio && !hasVideo) ||
      (this.expectedStreamType === 'video' && hasVideo);

    if (!isMatchingType) {
      console.log('[DoctorStreamReceiver] Tipo de stream não corresponde ao esperado, ignorando');
      // Não processa este stream pois não é para este componente
      return;
    }
  ...
  getDisplayStreamType(): 'auscultation' | 'video' {
    return this.streamType || this.expectedStreamType;
  }

  getStreamTitle(): string {
    const type = this.getDisplayStreamType();
