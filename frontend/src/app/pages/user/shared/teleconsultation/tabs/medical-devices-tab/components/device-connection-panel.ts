...
export class DeviceConnectionPanelComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Observa solicitação do médico para atualizar sinais vitais
    this.subscriptions.add(
      this.syncService.vitalsUpdateRequested$.subscribe(() => {
        console.log('[DeviceConnectionPanel] Médico solicitou atualização de sinais vitais');
        // Reenvia os sinais vitais atuais
        this.resendVitals();
      })
    );
  ...
  /**
   * Reenvia os sinais vitais atuais para o médico
   */
  private resendVitals(): void {
    const values = this.vitalsForm.value;
    const vitals = {
      spo2: values.spo2 || undefined,
      pulseRate: values.pulseRate || undefined,
      temperature: values.temperature || undefined,
      weight: values.weight || undefined,
      systolic: values.systolic || undefined,
      diastolic: values.diastolic || undefined,
      heartRate: values.heartRate || undefined
    };

    // Verifica se há algum dado para enviar
    const hasData = Object.values(vitals).some(v => v !== undefined);
    if (hasData) {
      console.log('[DeviceConnectionPanel] Reenviando sinais vitais:', vitals);
      this.syncService.sendVitalSigns(vitals).catch(err => {
        console.error('[DeviceConnectionPanel] Erro ao reenviar vitais:', err);
      });
    }
  }
  ...
  async saveVitals(): Promise<void> {
    ...
    try {
      ...
      // Marca como salvo imediatamente após API retornar (não espera SignalR)
      this.lastSaved = new Date();
      this.isSaving = false;

      // 2. Envia via SignalR para o médico em tempo real (fire-and-forget, não bloqueia)
      this.syncService.sendVitalSigns(biometrics).catch(signalRError => {
