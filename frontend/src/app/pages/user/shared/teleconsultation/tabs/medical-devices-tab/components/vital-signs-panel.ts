...
export class VitalSignsPanelComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Observa sinais vitais recebidos via SignalR
    this.subscriptions.add(
    ...
    // Inicia polling para buscar dados periodicamente (fallback se SignalR falhar)
    this.subscriptions.add(
      interval(POLLING_INTERVAL).subscribe(() => {
        this.pollForUpdates();
      })
    );
  }
  ...
  /**
   * Polling para buscar atualizações de sinais vitais do servidor
   * Usa HEAD request para verificar se há atualizações antes de buscar dados completos
   */
  private async pollForUpdates(): Promise<void> {
    if (!this.appointmentId) return;

    try {
      // Primeiro, verifica se há atualizações usando HEAD (mais leve)
      const checkUrl = `${environment.apiUrl}/appointments/${this.appointmentId}/biometrics${this.lastKnownTimestamp ? `?since=${encodeURIComponent(this.lastKnownTimestamp)}` : ''}`;
      
      const response = await fetch(checkUrl, { method: 'HEAD' });
      
      // 200 = há dados/atualizações, 204 = sem atualizações
      if (response.ok && response.status === 200) {
        await this.loadExistingData();
      }
    } catch (error) {
      // Erro silencioso no polling - não interrompe a experiência do usuário
      console.debug('[VitalSignsPanel] Erro no polling:', error);
    }
  }
  ...
  private async loadExistingData(): Promise<void> {
    ...
    try {
      ...
      if (data && this.hasAnyVitalData(data)) {
        // Armazena o timestamp para o polling incremental
        if (data.lastUpdated) {
