...
export class AuscultationPanelComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    // Observa solicitação do médico para iniciar ausculta
    this.subscriptions.add(
      this.syncService.streamRequested$.subscribe(async request => {
        if (request && request.deviceType === 'auscultation') {
          console.log('[AuscultationPanel] Médico solicitou ausculta');
          
          // Se já está transmitindo, reenvia a oferta automaticamente
          if (this.isStreaming) {
            console.log('[AuscultationPanel] Já transmitindo, reenviando oferta...');
            try {
              await this.syncService.resendCurrentStreamOffer();
              console.log('[AuscultationPanel] Oferta reenviada com sucesso');
            } catch (error) {
              console.error('[AuscultationPanel] Erro ao reenviar oferta:', error);
            }
          } else {
            // Se não está transmitindo, mostra notificação para o paciente
            this.showDoctorRequest = true;
            this.doctorRequestMessage = 'O médico solicitou que você inicie a ausculta';
  ...
  async startTransmissionToDoctor(): Promise<void> {
    console.log('[Auscultation] startTransmissionToDoctor chamado:', {
      appointmentId: this.appointmentId,
      isAuscultationActive: this.isAuscultationActive,
      selectedArea: this.selectedArea
    });

    if (!this.appointmentId) {
      this.deviceError = 'ID da consulta não definido';
      console.warn('[Auscultation] appointmentId não definido');
      return;
    }

    if (!this.isAuscultationActive) {
      this.deviceError = 'Inicie a ausculta primeiro';
      console.warn('[Auscultation] ausculta não está ativa');
      return;
    }

    const activeStream = this.streamingService.getActiveStream();
    console.log('[Auscultation] activeStream:', {
    ...
    if (!activeStream) {
      ...
      console.warn('[Auscultation] stream de áudio não disponível');
      return;
    }

    try {
      // Inicia streaming via WebRTC
      console.log('[Auscultation] Iniciando syncService.startStreaming...');
      ...
      console.log('[Auscultation] Transmissão ao médico iniciada com sucesso');
    } catch (error: any) {
